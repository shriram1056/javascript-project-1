...
form.addEventListener("submit", function (e) {
  //submit event – initiates its submission to the server.
  e.preventDefault();

  const value = parseInt(amount.value);//the value from keyboard int is string
  const random = Math.floor(Math.random() * text.length);

  //alphabet-isNan()- returns true if not a number
  // negatives 
  //2 digit number= since there are only 10 elements in array
  if (isNaN(value) || value < 0 || value > 9) {
    result.innerHTML = `input you have given is not a number`;
  } else {
    let tempText = text.slice(0, value);//start and no of values from start
    tempText = tempText
      .map(function (item) {
